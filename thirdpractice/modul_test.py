...
from practie_3 import Generator
...
def test_custom_list_generator():
    custom_list = CustomList()
    generator = Generator()
    spider_list = generator.generate_1000()
    for spider in spider_list:
        custom_list.append(spider)
    assert len(custom_list) == 1000
    assert custom_list[0].name == spider_list[0].name
    custom_list[0] = spider_list[1]
    assert custom_list[0].name == spider_list[1].name
    horse_list = generator.generate_10_000()
    custom_list_10k = CustomList()
    for spider in horse_list:
        custom_list_10k.append(spider)
    for spider in horse_list[::-1]:
        custom_list_10k.insert(0, spider)
    for spider in horse_list:
        custom_list_10k.append(spider)
    center_spider = custom_list_10k[len(custom_list_10k) // 2]
    for i, _ in enumerate(custom_list_10k):
        custom_list_10k[i] = center_spider
